...
ExtDef      :   Specifier ExtDecList SEMI  {$$=CreatNonTreeNode("Def"); AddNode($$,$1, $2, NULL);}
            ...
            |   Specifier FunDec CompSt	
            {
                ...
                */
                $$=$1; TreeNode *ptr = CreatNonTreeNode("NewScope");
                ptr->sibling=$2->child->sibling;
                $2->child->sibling = ptr;
...
Exp         :   Exp ASSIGNOP Exp	{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}   	    
            |   Exp AND Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}            
            |   Exp OR Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}          
            |   Exp RELOP Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}      
            |   Exp PLUS Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}    
            |   Exp MINUS Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}           
            |   Exp STAR Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}         
            |   Exp DIV Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$2); AddNode($2, $1, $3, NULL);}              
            |   LP Exp RP		{$$=$2;}             
            |   MINUS Exp		{$$=CreatNonTreeNode("Exp");AddNode($$,$1); AddNode($1, $2, NULL);}                  
            |   NOT Exp		      {$$=CreatNonTreeNode("Exp");AddNode($$,$1); AddNode($1, $2, NULL);}           
            |   ID LP Args RP		{$$=$1; AddSibling($1,$3, NULL);}          
            |   ID LP RP		{$$=$1;}             
